...
namespace CosmosDbDiagAndFeed.EventHubUtils
{
    public class SimpleEventProcessor : IEventProcessor
    {
        ...
        public Task ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
        {
            foreach (var eventData in messages)
            {
                ...
                var fileName = "eventhub." + (DateTime.UtcNow - new DateTime(1970, 1, 1)).TotalSeconds.ToString();
                File.WriteAllText(Path.Combine(LifetimeEventsHostedService.localLogdirectory, fileName), data);
                LifetimeEventsHostedService._logger.LogInformation("Event file written in " + LifetimeEventsHostedService.localLogdirectory + "/" + fileName + " at " + DateTime.Now);
            }
