...
CosmosDbDiagAndFeed enables you to monitor and audit [Azure Cosmos DB](https://aka.ms/cosmosdb) activity and access a collection's change feed.  It's easy to download Cosmos DB diagnostic logs and a collection's change feed to a Windows, macOS, or Linux computer.  
...
* Creates and configures an Azure Blob Storage account, and within that Storage account a Storage Container, which will be used to manage the Event Hub synchronization state of the .NET Core client application
...
*  [Cosmos DB Change Feed Processor](https://www.nuget.org/packages/Microsoft.Azure.DocumentDB.ChangeFeedProcessor), part of the Cosmos DB .NET SDK, which integrates with a dedicated Azure Cosmos DB collection to support checkpointing and parallel processing
